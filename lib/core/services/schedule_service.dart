...
///
/// Path: `schedules/{pushKey}` — the ESP32 polls this node and activates
/// the corresponding actuator at the specified time.
class ScheduleService {
  ...
  Stream<List<PumpSchedule>> get schedulesStream {
    return _db.ref('schedules').onValue.map((event) {
  ...
  Future<void> addSchedule(PumpSchedule schedule) {
    return _db.ref('schedules').push().set(schedule.toMap());
  }

  Future<void> updateSchedule(PumpSchedule schedule) {
    return _db.ref('schedules/${schedule.id}').update(schedule.toMap());
  }

  Future<void> deleteSchedule(String id) {
    return _db.ref('schedules/$id').remove();
  }
}
